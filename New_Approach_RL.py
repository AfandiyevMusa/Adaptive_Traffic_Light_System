...
def run_simulation(simulation_time):
    traci.start(['sumo-gui', '-c', 'ATLMS.sumocfg'])
    steps = 0
    ...
    while steps < total_time:
        ...
        for tls_id in traci.trafficlight.getIDList():
            if tls_id != 'JM1':
                continue  # Focus on the 'JM1' traffic light ID
        ...
        # Decrease epsilon every 10 steps, ensuring it doesn't go below 0.1
        # if steps % 10 == 0 and epsilon > 0.1:
...
if __name__ == "__main__":
    ...
    result_df.to_csv('CARCHANGE_10_26_DEMO_WithRL.csv', index=False)
